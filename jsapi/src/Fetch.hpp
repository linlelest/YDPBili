...
public:
    ...
    FetchOptions(std::string method = "GET",
        ...
        : method(method), headers(headers), body(body),
          stream(stream), streamCallback(streamCallback),
          rawStream(false), rawStreamCallback(nullptr), timeout(timeout),
          cancelled(cancelled) {}
};
...
private:
    ...
    static size_t RawWriteCallback(void *contents, size_t size, size_t nmemb, void *userdata);
